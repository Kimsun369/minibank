...
const BalanceCard: React.FC<BalanceCardProps> = ({ visible }) => {
  const { account } = useBank()

  if (!account) return null

  return (
    <div className="relative h-56 bg-gradient-to-br from-primary-600 to-primary-800 rounded-2xl p-8 text-white overflow-hidden group cursor-pointer transition-smooth hover:shadow-2xl">
      ...
      <div className="relative z-10">
        ...
        <div className="mb-4">
          ...
          <p className="text-4xl font-bold">
            {visible ? `$${account.balance.toLocaleString('en-US', { minimumFractionDigits: 2 })}` : '••••••'}
          </p>
        </div>
